...
- **Mail mode**: right-click empty space in a lada's tab for "Convert to mail" to turn it into a read-only Gmail inbox view — a lada with a single Mail-mode tab is effectively a mail-only widget. Shows the 15 most recent Inbox messages (sender, subject, snippet, unread state), refreshed automatically every 5 minutes in the background regardless of which lada or tab is currently focused; clicking a message opens it in the default browser. One shared Google account for the whole app, not per-tab. Requires a one-time setup outside the app: a personal Google Cloud project with the Gmail API enabled and a "Desktop app" OAuth client, whose credentials go into `%AppData%\Lada\google_client_secret.json` as `{"client_id": "...", "client_secret": "..."}` (see `docs/superpowers/specs/2026-08-12-mail-widget-design.md` for the exact steps). Because that Google Cloud project stays in "Testing" publishing status (avoiding Google's app-verification process, disproportionate for a single-user tool), the connection needs re-approving roughly every 7 days — a one-click reconnect banner appears in the tab rather than losing the last-known message list.
...
- Mail mode: a background poll that fails doesn't show anything distinct from a normal loading/empty state. `Strings.MailLastUpdateFailed` exists for this but isn't wired into the UI yet — only the separate "reconnect needed" banner (for the ~7-day Google re-auth) is.
...
- [x] **Standalone widgets**: done — "Nouveau widget" (tray menu, and any lada's own right-click menu) creates a dedicated single-component window (clock, timer, disk, battery, memory, CPU, GPU, or network — now called "components" to keep the two concepts distinct), closer to an iOS home-screen widget: fixed size, optional title/icon chrome (global toggle), draggable anywhere on it, deleting it deletes the whole window. Still a normal lada under the hood, so it inherits theme/Perspective 3D/HUD glow/magnetism for free.
